...
import { format, addDays, subDays, isSameDay, parseISO, isToday, getDay } from "date-fns"
...
export function NewWeeklyCalendar({ onAppointmentClick, userRole, onEditAppointment }: NewWeeklyCalendarProps) {
  ...
  const [currentDateTime, setCurrentDateTime] = useState(getCurrentEcuadorTime())
  ...
  const shouldShowTimeLine = () => {
    ...
    return isSameDay(currentDate, todayInEcuador) && now.getHours() >= 9 && now.getHours() <= 17
  }
  ...
  const branches = [
    { id: "principal", name: "Sucursal Principal", address: "Av. Principal 123" },
  ...
  const getNotificationStats = () => {
    const reservadas = appointments.filter(apt => apt.status === "programada").length
    const canceladas = appointments.filter(apt => apt.status === "cancelada").length  
    const postergadas = appointments.filter(apt => apt.status === "no_asistio").length
  ...
  const formatDateTime = (date: Date) => {
    const dateStr = formatInTimeZone(date, TIMEZONE, "dd/MM/yyyy", { locale: es })
    const timeStr = formatInTimeZone(date, TIMEZONE, "hh:mm:ss", { locale: es })
    const ampm = formatInTimeZone(date, TIMEZONE, "a", { locale: es }).toUpperCase()
  ...
  if (loading) {
    return (
      <div className="new-calendar-container" style={{display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '600px'}}>
        <div style={{color: '#6b7280', fontSize: '1.125rem'}}>Cargando calendario...</div>
      </div>
    )
  }
    ...
    <>
      ...
      <div className="arvi-header">
        <div className="arvi-header-content">
          <div className="arvi-logo" onClick={() => handleSectionChange("inicio")}>
            ARVI VET
          </div>
          ...
          <div className="arvi-user-controls">
            ...
            <div className="arvi-datetime" key={currentDateTime.getTime()}>
              ...
              <span>{formatDateTime(currentDateTime).timeStr} {formatDateTime(currentDateTime).ampm}</span>
            </div>
            ...
            <div className="arvi-notification-container" ref={notificationRef}>
              ...
              >
                ...
                {getNotificationStats().reservadas > 0 && (
                  <span className="notification-badge">{getNotificationStats().reservadas}</span>
                )}
                {showTooltip && getNotificationStats().reservadas > 0 && (
                  <div className="notification-tooltip">
                    Tienes {getNotificationStats().reservadas} cita{getNotificationStats().reservadas !== 1 ? 's' : ''} nueva{getNotificationStats().reservadas !== 1 ? 's' : ''}
                  </div>
                )}
              </button>
                ...
                <div className="arvi-dropdown notification-dropdown apple-popover">
                  ...
                  <div className="notification-stats">
                    <div className="stat-item">
                      <span className="stat-number">{getNotificationStats().reservadas}</span>
                      <span className="stat-label">Reservadas</span>
                    </div>
                    <div className="stat-item">
                      <span className="stat-number">{getNotificationStats().canceladas}</span>
                      <span className="stat-label">Canceladas</span>
                    </div>
                    <div className="stat-item">
                      <span className="stat-number">{getNotificationStats().postergadas}</span>
                    ...
                    {appointments.slice(0, 5).map((appointment) => (
                      <div key={appointment.id} className="notification-item">
                        <div className="notification-content">
                          <div className="notification-title">{appointment.owner.name}</div>
                          <div className="notification-subtitle">
                            {appointment.pet.name} - {getTypeText(appointment.type)}
                          </div>
                          <div className="notification-time">
                            {format(parseISO(appointment.date), "dd/MM", { locale: es })} - {appointment.time}
                          </div>
                        </div>
                        <div className={`notification-status ${appointment.status}`}>
            ...
            <div className="arvi-branch-container" ref={branchRef}>
              ...
              >
                <MapPin size={20} />
                <span className="branch-text">{branches.find(b => b.id === selectedBranch)?.name || 'Sucursal'}</span>
                <ChevronDown size={14} />
              </button>
            ...
            <div className="arvi-profile-container" ref={profileRef}>
              ...
              >
                {userPhoto ? (
                  <img src={userPhoto} alt="Profile" className="profile-photo apple-avatar" />
                ) : (
                ...
                <div className="arvi-dropdown profile-dropdown apple-popover">
                  ...
                  <div className="profile-info">
                    <div className="profile-item">
                      ...
                      <span className="profile-value">{userData?.name || 'Usuario'}</span>
                    </div>
                    <div className="profile-item">
                    ...
                    <div className="profile-item">
                      ...
                      <span className="profile-value">{userData?.id || 'N/A'}</span>
                    </div>
                  </div>
      ...
      <div className="new-calendar-header">
        <div className="new-calendar-nav">
          ...
          <div className="new-calendar-nav-arrows">
            <button className="new-calendar-nav-arrow" onClick={() => navigateDate('prev')}>
              <ChevronLeft size={16} />
            </button>
            <button className="new-calendar-nav-arrow" onClick={() => navigateDate('next')}>
              <ChevronRight size={16} />
            </button>
          </div>
          <h1 className="new-calendar-date-title">
            {format(getDateForDayIndex(0), "d", { locale: es })} - {format(getDateForDayIndex(5), "d 'de' MMMM, yyyy", { locale: es })}
          </h1>
        </div>
          ...
          {weekDays.map((day, index) => {
            ...
            return (
              <div key={index} className="new-calendar-day-column">
                ...
                <div className="new-calendar-day-name">
                  {day.name}
                </div>
              </div>
            )
        ...
        <div className="new-calendar-time-grid">
          ...
          {timeSlots.map((timeSlot, timeIndex) => (
            <div key={timeSlot} className="new-calendar-time-row">
              <div className="new-calendar-time-label">
                {timeSlot}
              </div>
              {weekDays.map((_, dayIndex) => {
                const dayAppointments = getAppointmentsForTimeAndDay(timeSlot, dayIndex)
                return (
                      ...
                      >
                        <div className="new-calendar-appointment-content">
                          ...
                          <div className="new-calendar-appointment-time">
                            {appointment.time} - {format(new Date(new Date().setHours(parseInt(appointment.time.split(':')[0]), parseInt(appointment.time.split(':')[1]) + appointment.duration)), 'HH:mm')}
                          </div>
                        </div>
      ...
      {showModal && selectedAppointment && (
        <div className="new-calendar-modal" onClick={closeModal}>
          <div className="new-calendar-modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="new-calendar-modal-header">
              <h2 className="new-calendar-modal-title">{selectedAppointment.owner.name}</h2>
              <button className="new-calendar-modal-close" onClick={closeModal}>
                <X size={20} />
              </button>
            </div>
            ...
            <div className="new-calendar-modal-body">
              <div className="new-calendar-modal-field">
                ...
                <div className="new-calendar-modal-field-content">
                  <div className="new-calendar-modal-field-label">Fecha y hora</div>
                  <div className="new-calendar-modal-field-value">
                    {format(parseISO(selectedAppointment.date), "EEEE dd 'de' MMMM", { locale: es })} - {selectedAppointment.time} a{' '}
                    {format(new Date(new Date().setHours(parseInt(selectedAppointment.time.split(':')[0]), parseInt(selectedAppointment.time.split(':')[1]) + selectedAppointment.duration)), 'HH:mm')} hrs
                  </div>
                </div>
              </div>
              ...
              <div className="new-calendar-modal-field">
                ...
                <div className="new-calendar-modal-field-content">
                  <div className="new-calendar-modal-field-label">Se atenderá con</div>
                  <div className="new-calendar-modal-field-value">
              ...
              <div className="new-calendar-modal-field">
                ...
                <div className="new-calendar-modal-field-content">
                  <div className="new-calendar-modal-field-label">Teléfono</div>
                  <div className="new-calendar-modal-field-value">{selectedAppointment.owner.phone}</div>
                </div>
              </div>
              ...
              <div className="new-calendar-modal-field">
                ...
                <div className="new-calendar-modal-field-content">
                  ...
                  <div className="new-calendar-modal-field-value">{selectedAppointment.reason}</div>
                </div>
              </div>
              ...
              {selectedAppointment.notes && (
                <div className="new-calendar-modal-field">
                  <FileText className="new-calendar-modal-field-icon" />
                  <div className="new-calendar-modal-field-content">
                    <div className="new-calendar-modal-field-label">Comentario interno</div>
                    <div className="new-calendar-modal-field-value">{selectedAppointment.notes}</div>
                  </div>
                </div>
              )}

              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '1.5rem' }}>
                <div className={`new-calendar-status-indicator ${selectedAppointment.status}`}>
                  <div className={`new-calendar-status-dot ${selectedAppointment.status}`}></div>
