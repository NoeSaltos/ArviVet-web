  ...
  return (
    <div className="card">
      ...
      <div className="card-header">
        ...
        <h2 className="card-title">{getUserTypeTitle()}</h2> {/* Título del formulario */}
      ...
      <div className="card-content">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="form">
          ...
          <div className="form-item">
            <label htmlFor="email" className={`form-label ${form.formState.errors.email ? "error" : ""}`}>
              Email
            </label>
            <input
              id="email"
            ...
            />
            {form.formState.errors.email && <p className="form-message">{form.formState.errors.email.message}</p>}{" "}
            {/* Mensaje de error */}
          </div>
          {/* Campo de Contraseña */}
          <div className="form-item">
            <label htmlFor="password" className={`form-label ${form.formState.errors.password ? "error" : ""}`}>
              Contraseña
            </label>
            <div className="input-container">
              ...
              >
                {showPassword ? <EyeOff className="icon" /> : <Eye className="icon" />} {/* Icono de ojo */}
              </button>
            </div>
            {form.formState.errors.password && <p className="form-message">{form.formState.errors.password.message}</p>}{" "}
            {/* Mensaje de error */}
          </div>
          {/* Botón de Enviar */}
          <button type="submit" className="button button-default button-full-width" disabled={isLoading}>
