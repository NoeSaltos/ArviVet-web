...
export const bulkScheduleSchema = z.array(createScheduleSchema).min(1, 'At least one schedule is required');
...
export const createBlockSchema = z.object({
  ...
  start_time: timeSchema,
  end_time: timeSchema,
  reason: z.string().min(3, 'Reason must be at least 3 characters').max(255, 'Reason must be less than 255 characters'),
}).refine(
  (data) => data.start_time < data.end_time,
  {
    message: 'Start time must be before end time',
...
export const createHolidaySchema = z.object({
  name: z.string().min(3, 'Holiday name must be at least 3 characters').max(100, 'Holiday name must be less than 100 characters'),
  date: dateSchema,
});
...
export const bulkHolidaySchema = z.array(createHolidaySchema).min(1, 'At least one holiday is required');
...
export const createAppointmentSchema = z.object({
  ...
  vet_id: positiveIntSchema,
  status: z.enum(['programada', 'confirmada', 'en_curso', 'completada', 'cancelada', 'no_asistio', 'reprogramada']).optional(),
});
...
export const calendarFiltersSchema = z.object({
  ...
  speciality_id: positiveIntSchema.optional(),
  branch_id: positiveIntSchema.optional(),
  status: z.array(z.enum(['programada', 'confirmada', 'en_curso', 'completada', 'cancelada', 'no_asistio', 'reprogramada'])).optional(),
  date_range: z.object({
    start: dateSchema,
    end: dateSchema,
  }).refine(
...
export const searchQuerySchema = z.object({
  search_term: z.string().min(2, 'Search term must be at least 2 characters').max(100),
...
export const scheduleFormSchema = z.object({
  ...
  (data) => data.weekdays.some(day => day.enabled),
  {
...
export function validateScheduleConflict(
  ...
  existingSchedules: Array<{ weekday: number; start_time: string; end_time: string }>
): boolean {
  return !existingSchedules.some(existing => 
    existing.weekday === newSchedule.weekday &&
    !(newSchedule.end_time <= existing.start_time || newSchedule.start_time >= existing.end_time)
  );
}
...
export function validateBlockConflict(
  ...
  return !existingBlocks.some(existing => 
    ...
    !(newBlock.end_time <= existing.start_time || newBlock.start_time >= existing.end_time)
  );
}
 ...
 */
export function validateWorkingHours(start_time: string, end_time: string): boolean {
 ...
 */
export function validateReasonableFutureDate(date: string, maxMonthsAhead: number = 12): boolean {
  const inputDate = new Date(date);
